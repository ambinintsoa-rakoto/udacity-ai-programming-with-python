...
#                                                                             
# PROGRAMMER: Ambinintsoa Jocelyn RAKOTONIRINA
...
def print_results(results_dic, results_stats_dic, model, 
    ...
    if(print_incorrect_dogs == True) and (results_stats_dic['n_correct_dogs'] + results_stats_dic['n_correct_notdogs'] != results_stats_dic['n_images']):
        print("Labels aren't in agreement regarding whether or not an image is of a dog.")
        for key in results_dic:
            if sum(results_dic[key][3:]) == 1:
                print("Truth : {}  Classified: {} ".format(results_dic[key][0], results_dic[key][1]))
                
    #Misclassified Breed's of Dog n_correct_dogs != n_correct_breed
    if(print_incorrect_breed == True) and (results_stats_dic['n_correct_dogs'] != results_stats_dic['n_correct_breed']):
        print("Labels indicate that the image is a dog; but, labels aren't in agreement regarding the dog's breed.")
        for key in results_dic:
